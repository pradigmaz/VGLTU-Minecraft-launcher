...
# 2. Запускаем, если сервис не активен
if ! sudo systemctl is-active --quiet nginx; then
    log_warn "Nginx не активен. Принудительный запуск..."
    sudo systemctl start nginx
else
    # 3. Перезагружаем, если уже активен
    log_info "Финальная перезагрузка Nginx..."
