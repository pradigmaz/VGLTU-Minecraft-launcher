...
docker inspect faculty_backend
```
...
docker-compose exec postgres psql -U launcher -d faculty_launcher

# Выполнить SQL команду
docker-compose exec postgres psql -U launcher -d faculty_launcher -c "SELECT * FROM users;"

# Создать резервную копию
docker-compose exec postgres pg_dump -U launcher faculty_launcher > backup.sql

# Восстановить из резервной копии
docker-compose exec -T postgres psql -U launcher faculty_launcher < backup.sql
```
...
docker-compose exec minio mc mb minio/faculty-storage
...
```yaml
DATABASE_URL: postgresql+asyncpg://launcher:dev_secret_password@postgres/faculty_launcher
